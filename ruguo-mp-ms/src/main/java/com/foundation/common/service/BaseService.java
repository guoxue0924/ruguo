...
public abstract class BaseService {
	 ...
	 * @return 标准连接条件对象
	 */
	public static String dataScopeFilter(User user, String archiveAlias) {
		...
		// for (String aa : StringUtils.split(archiveAlias, ",")) {
		// if (!dataScope.contains(r.getDataScope()) &&
		// StringUtils.isNotBlank(aa)) {
		// if (Role.DATA_SCOPE_ALL.equals(r.getDataScope())) {
		// isDataScopeAll = true;
		// } else if (Role.DATA_SCOPE_SELF.equals(r.getDataScope())) { // 本服务机构
		// sqlString.append(" OR " + aa + ".service_organization_id = '" +
		// user.getOrgId() + "'");
		// } else if (Role.DATA_SCOPE_SELF_AND_ASSIS.equals(r.getDataScope())) {
		// // 本服务机构与协办检查机构
		// sqlString.append(" OR " + aa + ".service_organization_id = '" +
		// user.getOrgId() + "'");
		// sqlString.append(" OR " + aa + ".assist_organization_id = '" +
		// user.getOrgId() + "'");
		// } else if (Role.DATA_SCOPE_SELF_AND_FOLLOW.equals(r.getDataScope()))
		// { // 本服务机构与预期随访机构
		// sqlString.append(" OR " + aa + ".service_organization_id = '" +
		// user.getOrgId() + "'");
		// sqlString.append(" OR " + aa + ".followup_organization_id = '" +
		// user.getOrgId() + "'");
		// } else if
		// (Role.DATA_SCOPE_SELF_AND_ASSIS_AND_FOLLOW.equals(r.getDataScope()))
		// { // 本服务机构、协办检查机构与预期随访机构
		// sqlString.append(" OR " + aa + ".service_organization_id = '" +
		// user.getOrgId() + "'");
		// sqlString.append(" OR " + aa + ".assist_organization_id = '" +
		...
		// sqlString.append(" OR " + aa + ".followup_organization_id = '" +
		// user.getOrgId() + "'");
		// } else if (Role.DATA_SCOPE_MANAGER.equals(r.getDataScope())) { //
		// 本管理机构
		// //sqlString.append(" OR " + aa + ".zone_code = '" +
		// user.getOwnZoneCode() + "'");
		// String level = user.getOrgLevel();
		// if (CommonConstant.DictZoneLevel.provice.equals(level)) {
		// sqlString.append(" OR ( " + aa + ".province_code = '" +
		// user.getOwnZoneCode() + "'" + " and " + aa + ".level='" + level +
		// "')");
		// } else if (CommonConstant.DictZoneLevel.city.equals(level)) {
		// sqlString.append(" OR ( " + aa + ".city_code = '" +
		// user.getOwnZoneCode() + "'" + " and " + aa + ".level='" + level +
		// "')");
		// } else if (CommonConstant.DictZoneLevel.county.equals(level)) {
		// sqlString.append(" OR ( " + aa + ".county_code = '" +
		// user.getOwnZoneCode() + "'" + " and " + aa + ".level='" + level +
		// "')");
		// } else if (CommonConstant.DictZoneLevel.town.equals(level)) {
		// sqlString.append(" OR ( " + aa + ".town_code = '" +
		// user.getOwnZoneCode() + "'" + " and " + aa + ".level='" + level +
		// "')");
		// }
		// } else if
		// (Role.DATA_SCOPE_MANAGER_AND_CHILD.equals(r.getDataScope())) { //
		// 本管理机构及下属管理机构
		// String level = user.getOrgLevel();
		// if (CommonConstant.DictZoneLevel.provice.equals(level)) {
		// sqlString.append(" OR " + aa + ".province_code = '" +
	...
	public static String dataScopeFilter(User user, String officeAlias, String userAlias) {
		...
		if (!user.isAdmin()) {
			/*
			 * boolean isDataScopeAll = false; for (Role r : user.getRoleList())
			 * { for (String oa : StringUtils.split(officeAlias, ",")) { if
			 * (!dataScope.contains(r.getDataScope()) &&
			 * StringUtils.isNotBlank(oa)) { if
			 * (Role.DATA_SCOPE_ALL.equals(r.getDataScope())) { isDataScopeAll =
			 * true; } else if (Role.DATA_SCOPE_SELF.equals(r.getDataScope())) {
			 * // 本服务机构 // sqlString.append(" OR " + oa + ".id = '" +
			 * user.getCompany().getId() + "'"); // sqlString.append(" OR " + oa
			 * + ".parent_ids LIKE '" + user.getCompany().getParentIds() +
			 * user.getCompany().getId() + ",%'"); } else if
			 * (Role.DATA_SCOPE_SELF_AND_ASSIS.equals(r.getDataScope())) { //
			 * 本服务机构与协办检查机构 // sqlString.append(" OR " + oa + ".id = '" +
			 * user.getCompany().getId() + "'"); // // 包括本公司下的部门
			 * （type=1:公司；type=2：部门） // sqlString.append(" OR (" + oa +
			 * ".parent_id = '" + user.getCompany().getId() + "' AND " + oa +
			 * ".type = '2')"); } else if
			 * (Role.DATA_SCOPE_SELF_AND_FOLLOW.equals(r.getDataScope())) { //
			 * 本服务机构与预期随访机构 // sqlString.append(" OR " + oa + ".id = '" +
			 * user.getOffice().getId() + "'"); // sqlString.append(" OR " + oa
			 * + ".parent_ids LIKE '" + user.getOffice().getParentIds() +
			 * user.getOffice().getId() + ",%'"); } else if
			 * (Role.DATA_SCOPE_SELF_AND_ASSIS_AND_FOLLOW.equals(r.getDataScope(
			 * ))) { // 本服务机构、协办检查机构与预期随访机构 // sqlString.append(" OR " + oa +
			 * ".id = '" + user.getOffice().getId() + "'"); } else if
			 * (Role.DATA_SCOPE_MANAGER.equals(r.getDataScope())) { // 本管理机构 //
			 ...
			 * +".id)"); } else if
			 * (Role.DATA_SCOPE_MANAGER_AND_CHILD.equals(r.getDataScope())) { //
			 * 本管理机构及下属管理机构 // String officeIds =
			 * StringUtils.join(r.getOfficeIdList(), "','"); // if
			 * (StringUtils.isNotEmpty(officeIds)){ // sqlString.append(" OR " +
			 * oa + ".id IN ('" + officeIds + "')"); // } // sqlString.
			 * append(" OR EXISTS (SELECT 1 FROM sys_role_office WHERE role_id = '"
			 * + r.getId() + "'"); // sqlString.append(" AND office_id = " + oa
			 * +".id)"); }
			 * 
			 * dataScope.add(r.getDataScope()); } } } //
			 * 如果没有全部数据权限，并设置了用户别名，则当前权限为本人；如果未设置别名，当前无权限为已植入权限 if
			 * (!isDataScopeAll){ if (StringUtils.isNotBlank(userAlias)){ for
			 * (String ua : StringUtils.split(userAlias, ",")){
			 * sqlString.append(" OR " + ua + ".id = '" + user.getId() + "'"); }
			 * }else { for (String oa : StringUtils.split(officeAlias, ",")){
			 * //sqlString.append(" OR " + oa + ".id  = " +
			 * user.getOffice().getId()); sqlString.append(" OR " + oa +
			 * ".id IS NULL"); } } }else{ // 如果包含全部权限，则去掉之前添加的所有条件，并跳出循环。
	 ...
	 *            当前过滤的实体类
	 * @param sqlMapKey
	 *            sqlMap的键值，例如设置“dsf”时，调用方法：${sqlMap.dsf}
	 * @param officeWheres
	...
	public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String officeWheres, String userWheres) {
		...
		// userWheresUser user = entity.getCurrentUser();
		//
		// // 如果是超级管理员，则不过滤数据
		// if (user.isAdmin()) {
		// return;
		...
		//// sqlString.append(" AND EXISTS (SELECT 1 FROM sys_role_office
		// ro123456, sys_office o123456");
