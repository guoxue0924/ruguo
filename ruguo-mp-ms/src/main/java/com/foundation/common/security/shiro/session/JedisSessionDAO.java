 ...
 */
package com.foundation.common.security.shiro.session;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
	...
	public void update(Session session) throws UnknownSessionException {
		...
		try {
			...
			JedisUtils.hset(sessionKeyPrefix, session.getId().toString(), principalId + "|" + session.getTimeout() + "|" + session.getLastAccessTime().getTime());
			JedisUtils.set(JedisUtils.getBytesKey(sessionKeyPrefix + session.getId()), JedisUtils.toBytes(session));
			
			// 设置超期时间
			int timeoutSeconds = (int)(session.getTimeout() / 1000);
			JedisUtils.expire((sessionKeyPrefix + session.getId()), timeoutSeconds);
	...
	public void delete(Session session) {
		...
		try {
			...
			JedisUtils.hdel(JedisUtils.getBytesKey(sessionKeyPrefix), JedisUtils.getBytesKey(session.getId().toString()));
			JedisUtils.del(JedisUtils.getBytesKey(sessionKeyPrefix + session.getId()));
				...
				if (StringUtils.isNotBlank(e.getKey()) && StringUtils.isNotBlank(e.getValue())){
					...
					if (ss != null && ss.length == 3){// jedis.exists(sessionKeyPrefix + e.getKey())){
						...
						// SESSION验证失败
							...
							JedisUtils.hdel(sessionKeyPrefix, e.getKey());
						}
					}
					// 存储的SESSION不符合规则
					else{
