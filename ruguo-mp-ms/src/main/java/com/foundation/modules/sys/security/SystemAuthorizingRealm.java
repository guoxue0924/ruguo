...
import com.foundation.modules.sys.web.LoginController;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		//设置
		 UserUtils.setByLoginName(token.getUsername());
		...
		if (user != null) {
			if (Global.NO.equals(user.getLoginFlag())){
				throw new AuthenticationException("msg:该帐号未启用.");
			}
			if (Global.STOP.equals(user.getLoginFlag())){
				throw new AuthenticationException("msg:该帐号已停用.");
			}
			User user1 = systemService.getUser(user.getId());
			byte[] salt = Encodes.decodeHex(user.getPassword().substring(0,16));
			return new SimpleAuthenticationInfo(new Principal(user1, token.isMobileLogin()), 
...
//		private Map<String, Object> cacheMap;
		...
		public Principal(User user, boolean mobileLogin) {
			...
			this.zoneCode = user.getOwnZoneCode();
		}
