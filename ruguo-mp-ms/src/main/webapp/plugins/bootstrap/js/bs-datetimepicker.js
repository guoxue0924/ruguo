...
(function (factory) {
    ...
    var Datetimepicker = function (element, options) {
        ...
        // 制定是否在dialog里加载日期
        var modal = this.element.closest('div.modal-content');
        if (modal.length > 0) {
    ...
    Datetimepicker.prototype = {
        ...
        show: function (e) {
            if ($(e.target).closest('.date').find('input').is(':disabled'))return false;
            var dateinput = $(e.target).closest('.date').find('input');
            
//          if (/^\d{4}\-\d{1,2}\-\d{1,2}$/.test(dateinput.val())) {
//              dateinput.val(dateinput.val().replace(/\-/g,''))
//          }
            dateinput.select();
            this.picker.show();
