						...
						<div class="form-group form-group-sm">
							...
							<div class="col-xs-7-sm">
								<input type="text" maxlength="64" name="parentName" disabled
									id="parentName" class="form-control" value="${result.fullName}">
							</div>
						</div>
					</div>
					<div class="col-xs-6">
				...
				<div class="col-xs-12-sm">
					<div class="col-xs-6">
						<div class="form-group form-group-sm" data-msg-direction="d">
							...
							<div class="col-xs-7-sm">
								<input type="text" maxlength="64" name="code" id="code" autocomplete="off"
									class="form-control" value="">
							</div>
						</div>
					</div>
								...
								<select class="selectpicker " name="zoneTag" id="zoneTags"
									...
									<c:forEach items="${fns:getDictList('DICT_ZONE_QUBS')}"
										...
										<option value="${DICT_ZONE_QUBS.value}"
											${DICT_ZONE_QUBS.value == opt.patient.level ?'selected':''}>${DICT_ZONE_QUBS.label}</option>
									</c:forEach>
								</select>
							</div>
						</div>
								...
								<select class="selectpicker " name="isPoorCounty"
									...
									<c:forEach items="${fns:getDictList('DICT_IDENTITY_FLAG')}"
										...
										<option value="${DICT_IDENTITY_FLAG.value}"
											${DICT_IDENTITY_FLAG.value == opt.patient.level  ?'selected':''}>${DICT_IDENTITY_FLAG.label}</option>
									</c:forEach>
								</select>
							</div>
						</div>
...
<script
	src="<%=request.getContextPath()%>/modules/dictzone/dictZoneAdd.js"></script>
