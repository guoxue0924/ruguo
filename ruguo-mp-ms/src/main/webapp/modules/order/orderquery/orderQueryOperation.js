        ...
        $(function () {
            ...
            function initGrid(){
            	grid = $("#orderGrid").bootgrid({
            		...
            		url: webAppPath + '/order/orderquery/orderrefundlist',
            		rowSelect: false,
            		selection: false,
            		multiSelect: false,
            		emptyPost: true,
            		...
            		formatters: {
            			...
            			"operation": function (column, row) {
            				...
            				var audit_html = "";
            				// 如果是申请退款状态，添加审核链接
            				if(row.orderStatus === common.CommonConstant["OrderStatus"]["applyForRefund"]){
            					if(row.approvalStatus === common.CommonConstant["COMMON_YES"]){
            						audit_html = " <a  style='opacity:0.4;' >审核</a>";
            					}else{
            						audit_html = " <a href='#' class=\"audit\" data-code='" + row.orderCode + "' >审核</a>";
            					}
            		...
            		grid.find(".audit").on("click", function (e) {
            			...
            			$.audit = BootstrapDialog.show({
            				...
            				message: function () {
            					var $message = $('<div></div>');
            					return $message.load(webAppPath + '/order/orderquery/audit?orderCode=' + orderCode);
            				}
            			});
            		});
            	});
